 ...
 * @Last Modified time: 2017-09-10 17:40:23
 * @desc: react-native-select
 */
...
export default class Select extends Component {
  static propTypes = {
    ...
    containerStyle: PropTypes.object, // select外部容器 自定义样式
    contentStyle: PropTypes.object, // select内容 自定义样式
  ...
  initDataSource = (dataSource, defaultVal, labelName, valName, isRemoveWhileSelected, multiple) => {
    if (!defaultVal) return dataSource.slice().map(e => {
      e.isChecked = false
      return e
    })
   ...
   */
  setVisible = (status) => {
    console.log('status is : ', status, 'conf: ', this.state.conf)
