 ...
 * @Last Modified time: 2017-10-09 09:21:00
...
import { isStr, isBool, isIncludes, cbFns, analysisStyle } from './utils'
...
export default class SuperChooser extends Component {
  static propTypes = {
    ...
    renderTitle: PropTypes.element, // 选择框头部元素
  ...
  initConf = (props) => {
    ...
    if (typeof containerStyle === 'object' && containerStyle.top && !containerStyle.height) {
    ...
    return {
      ...
      containerStyle: analysisStyle(containerStyle),
      contentStyle: analysisStyle(contentStyle),
      checkedColor: isStr(checkedColor, '#2296F3'),
      renderTitle,
      itemStyle: analysisStyle(itemStyle),
      listStyle: analysisStyle(listStyle),
      duration: duration >= 0 ? duration : 222,
      position: isIncludes(['top', 'bottom'], position, 'top'),
      numColumns: numColumns > 1 ? Number(numColumns) : 1,
      btnContainerStyle: analysisStyle(btnContainerStyle),
      columnWrapperStyle: analysisStyle(columnWrapperStyle),
      multiple: _multiple,
  ...
  _toggleAnimation = (status, fn) => {
    Animated.timing(
      ...
      fn && fn()
      this._limitSubmit(status)
    })
  }

  /**
   * @desc 提交标志, 防止快速点确认按钮, 导致重复提交
   * @param  {Boolean} status
   */
  _limitSubmit = status => {
    this.submitFlag = status
  }
  ...
  render () {
    ...
    const { renderTitle, containerStyle, contentStyle, backgroundColor, onCancel, dataSource, position } = conf
